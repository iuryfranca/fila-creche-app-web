...
>(({ className, ...props }, ref) => {
  ...
  const [valueAccordion, setValueAccordion] = React.useState('')
    ...
    >
      ...
      <nav className="grid grid-rows-[50px] items-start gap-2">
        {siteConfig.routeConfig.map((item, index) => {
          ...
          function DropdownMenuRegisterAndQuery() {
            return (
              <>
                <Accordion
                  type="single"
                ...
                >
                  <AccordionItem value="register" className="border-none">
                    <AccordionTrigger
                      // onClick={() =>
                      //   valueAccordion !== 'register'
                      //     ? setValueAccordion('register')
                      //     : setValueAccordion('')
                      // }
                      className={cn(
                ...
                >
                  <AccordionItem value="query" className="border-none">
                    <AccordionTrigger
                      // onClick={() =>
                      //   valueAccordion !== 'query'
                      //     ? setValueAccordion('query')
                      //     : setValueAccordion('')
                      // }
                      className={cn(
                        'group flex items-center rounded-md px-3 py-2 text-sm font-medium text-primary-foreground transition duration-300 hover:bg-secondary hover:text-secondary-foreground',
                    ...
                    <AccordionContent>
                      <div className="ml-4 mt-2 flex flex-col gap-1">
                        {routesQuery.map((itemQuery, index) => {
                          return (
                            <Link
                              key={itemQuery.title}
                              href={
